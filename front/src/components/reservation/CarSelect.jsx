...
const CarSelect = ({ locationName = "반얀트리 호텔" }) => {
    ...
    useEffect(() => {
        const fetchCars = async () => {
            try {
                setLoading(true);
                const response = await fetch("https://mocaapp.net/api/rental/admin/cars");
    ...
    useEffect(() => {
        const fetchReservedCars = async () => {
            ...
            try {
                const response = await fetch(`https://mocaapp.net/api/reservations/reserved-cars?date=${date}`);
                if (!response.ok) {
